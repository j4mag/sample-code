classdef HybridPlayer < Player
    %HYBRIDPLAYER This is an clue game player that uses their clues
    %   collaboratively, but only gives clues when they need to.
    %   A hybrid player only gives clues when they run out of inventory,
    %   prioritizing friends that give the most clues.
    properties
        friendClueCount = zeros(1,0);
    end
    
    methods
        function self = HybridPlayer(id)
            %HYBRIDPLAYER Construct an instance of this class
            %   Inputs:
        ...
        function [players, self] = giveClue(self,players)
            %GIVECLUE this player gives a clue to another player. The clue
            ...
            for friendID = self.friendList
                ...
                possibleTrades = (players{friendID}.getClueList()==0 & self.myClues>0)...
                    .* self.getClueList().*sqrt(1+self.friendClueCount(self.friendList==friendID));
