classdef SecretivePlayer < Player
    %SECRETIVEPLAYER This is a clue game player that uses their clues
    %   selfishly.
    %   A secretive player differs from a selfish player in that they do not
    %   advertize their clue status to their friends. They appear to always
    %   have only clues given to them. Clues they obtain on their own do not appear
    %   in their getClueList().
    
    methods
        function self = SecretivePlayer(id)
            %SECRETIVEPLAYER Construct an instance of this class
            %   Inputs:
        ...
        function [players, self] = findClue(self,cluenum,players)
            ...
            if sum(self.myClues) < self.maxClues
                ...
                if all(self.myClues + self.rxClues > 0)
